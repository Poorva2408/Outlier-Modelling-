...
datM$Forensic_Label[datM$Forensic_Z > 1.78 & datM$Forensic_Z <= 2.5]  <- "Strong Performance" # adjust boundary here
print(datM$Forensic_Label[133])
...
print(audit)
# changeable
outlier1_k <- audit$Pareto_k[1] # -0.09361934
outlier2_k <- audit$Pareto_k[2] # -0.02605248
max_k_idx <- which.max(k_values)
print(max_k_idx) # id  = 134
outlier_report <- datM[max_k_idx, ]
print(paste("The structural limit of the model is being tested at Row:", max_k_idx))
print(outlier_report) 
#Index Avg Dist Drive Accuracy(%) accuracy_prop_M Anomaly_Status PIT_Score Forensic_Label Forensic_Z  Pareto_k     sq_res
#291   291    274.9         74.9         0.747736  System Normal   0.85575  System Normal   1.047693 0.1330855 0.01317094
print(paste("Pareto k for this point:", round(k_values[max_k_idx], 4)))
# Pareto k for this point: 0.1331
...
print(paste("Surprise Score:", round(swan_surprise, 4))) # -0.7956
...
print(avg_loglik - swan_loglik) # 2.386668
...
print(sensitivity_gap) # 2.484333
...
# method 1 : 
# check prior summary 
prior_summary(bayesian_golf_beta_M_3)
# Refit the model sampling only from the priors
prior_only_model <- update(bayesian_golf_beta_M_3, prior_PD = TRUE)

# 1. Ensure you have the prior-only draws
# (Assuming 4000 draws by default, we slice the first 100 for the visual)
y_rep_prior <- posterior_predict(prior_only_model)

# 2. Generate the Density Overlay (use 50 seasons over 100 for better visuals)
# This compares your observed 'y' (dark line) to 100 'hallucinated' seasons (light lines)
color_scheme_set("teal")
ppc_dens_overlay(
  y = datM$accuracy_prop_M, 
  yrep = y_rep_prior[1:20, ]
) + 
  scale_x_continuous(limits = c(0, 1)) +
  labs(
    title = "Prior Predictive Check: Structural Resolution Audit ",
    subtitle = "Dark teal: PGA Reality | Light teal: Prior Hallucinations(25 seasons)",
    x = "Driving Accuracy (Proportion)",
    y = "Density"
  ) +
  theme_minimal()

# Evaluation : 
#1. the Y-axis scale is capped at 30. This allows you to actually see the "U-shape" of your current priors.
#2. The Diagnostic: Even though it's intelligible, it's still a "Failure." Those light pink lines pooling at 0.00 and 1.00 prove 
# these priors are still High Entropy. They are predicting that golfers will either be "Perfect" or "Total Failures," with almost no 
# probability in the 65% "Reality Zone".

# method 2 : Gamma prior with y clipping 
# Prior Selection (Normal, student-t, exponential)
# Your bayesian_golf_beta_M_3 is using the following "Blurry" logic:
# Intercept ~ normal(0, 2.5): On the logit scale, a 2.5 scale is massive. It tells the model that a 0.1% or 99.9% accuracy is just as 
# plausible as a 65% accuracy. This is why your initial "hallucinations" spanned the entire X-axis.

# Coefficients ~ student_t(7, 0, 2.5): These are heavy-tailed priors for the distance penalty. They allow for "explosive" slopes where 
# accuracy could drop 20% for every single yard gained, which is non-physical for PGA pros.

# Auxiliary (phi) ~ exponential(1): This is the Primary Failure Point. An exponential prior with a rate of 1 favors values near zero. 
# Since low phi equals high entropy, your model is internally biased to see every golfer as a random noise generator. It effectively 
# masks the 8.43x Deficit by assuming the noise floor is naturally high.

library(brms)

hd_priors <- c(
  prior(normal(0.65, 0.05), class = "Intercept"), # Calibrate the DC Offset
  prior(normal(-0.005, 0.002), class = "b"),      # Constrain the slope
  prior(gamma(20, 0.1), class = "phi")            # Fix the Noise Floor
)

# Intercept: Sets the baseline performance (the 65% PGA mean).
# b: Sets the "Gain" or "Slope" for your predictors (like Avg Dist).
# phi: Sets the Precision/Resolution (the 8.43x deficit factor focus).

# Gamma Prior selection 
# Choosing Gamma parameters is a Signal-to-Noise Ratio (SNR) calculation. In Beta regression, phi represents precision; higher 
# values mean a tighter, "High-Definition" distribution.

# The Mean (The Floor): For a Gamma distribution, the mean is alpha / beta. Using 20 / 0.1 = 200 sets your expected precision at 200.
# This is a professional-grade resolution that prevents the "U-shaped" hallucinations seen in your earlier plots.

# The Shape (alpha = 20): By keeping the shape parameter high (>1), you ensure the probability density at phi = 0 is zero. 
# This "clamps" the entropy and prevents the model from assuming golfers have 0% accuracy.

# The Rate (beta = 0.1): This controls the "spread" of your assumption. It gives the model enough "mathematical tension" to stay 
# near phi=200 but enough flexibility to adjust if the PGA data suggests a higher noise floor.
...
# Refit to move from 1e+05 spikes to the HD Overlay
#gamma_prior_cal <- update(bayesian_golf_beta_M_3, prior = hd_priors, recompile = TRUE) ## Drop update 
datM$avg_dist_clean <- datM$`Avg Dist`

gamma_prior_cal <- brm(
  formula = accuracy_prop_M ~ avg_dist_clean, 
  data = datM, 
  family = brmsfamily("beta"),
  prior = hd_priors,
  sample_prior = "only", # Specifically for the Prior Predictive Check
  chains = 4, cores = 4, iter = 2000
)


y_rep_matrix <- posterior_predict(gamma_prior_cal)

color_scheme_set("#6a0dad")
ppc_dens_overlay(
  y = datM$accuracy_prop_M, 
  yrep = y_rep_matrix[1:50, ]
) + 
  scale_x_continuous(limits = c(0, 1)) +
  labs(
    title = "Prior Predictive Check: Structural Resolution Audit ",
    subtitle = "Dark purple: PGA Reality | Light purple: Gamma Prior Hallucinations(50 seasons)",
    x = "Driving Accuracy (Proportion)",
    y = "Density"
  ) +
  theme_minimal()

# You can apply HD priors to a low-res (constant phi) model, but it’s like putting high-octane fuel in a standard engine.
# Low-Res Model + HD Priors: Fixes the "blurry" hallucinations and centers the peak at 65%. However, it still enforces a single 
# phi for every golfer.
# HD Model (phi ~ Avg Dist): This is where you actually unmask the 8.43x Resolution Deficit. It allows phi to change based on 
# distance, proving that precision isn't constant across the field.



# prior mapping without using update 
# prior posterior distribution mapping 
# credible bands
# HD model 
# conclusion  
# Update your PATH for the current session
# Adjust "rtools44" if you installed a different version (e.g., rtools43)
